...
a host to monitor the utilization（利用率） of all links in the network. This
...
will modify(改进) the basic P4 program to process a source routed probe(探针)
...
four hosts connected to four switches that are wired up(连通) as they
would be in a single pod of a fat tree topology(胖树拓扑).
